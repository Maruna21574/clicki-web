...
export const Navbar: React.FC<NavbarProps> = ({ currentPage, onNavigate }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-[100] transition-all duration-700 ${isScrolled ? 'bg-black/90 backdrop-blur-xl py-4 border-b border-white/5' : 'bg-transparent py-10'}`}>
      <div className="max-w-[1440px] mx-auto px-10 flex justify-between items-center">
        <button onClick={() => onNavigate('home')} className="flex items-center group overflow-hidden">
          <div className="font-giaza text-4xl font-black text-[#F43182] tracking-tighter transition-transform duration-500 group-hover:-translate-y-full">
            CLICKI
          </div>
          <div className="font-giaza text-4xl font-black text-white tracking-tighter absolute transition-transform duration-500 translate-y-full group-hover:translate-y-0">
            CLICKI
          </div>
